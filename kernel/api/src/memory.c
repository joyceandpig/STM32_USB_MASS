...
uint32_t mem_free(void *rmem)
{
	...
	if (rmem == NULL)
	{
		u_printf(DBG,"mem_free(p == NULL) was called.\n");
	...
	if ((unsigned char*)rmem < (unsigned char*)ram || (unsigned char*)rmem >= (unsigned char*)ram_end)
	{
		...
		u_printf(DBG,"mem_free: illegal memory\n");
	...
	if (mem->used != 1)
	{
		...
		mem_slide_check(FALSE);
#endif
		u_printf(ERR,"mem_free err:%x", mem->used);
		return size;
		//while(1);
	}
	#endif
...
void *mem_trim(void *rmem, mem_size_t newsize)
{
	...
	if ((unsigned char*)rmem < (unsigned char*)ram || (unsigned char*)rmem >= (unsigned char*)ram_end)
	{
		...
		u_printf(DBG,"mem_trim: illegal memory\n");
		/* protect mem stats from concurrent access */
...
void *mem_malloc_ex(const char *name, mem_size_t size)
{
	...
	while (local_mem_free_count != 0)
		;
	#endif /* LWIP_ALLOW_MEM_FREE_FROM_OTHER_CONTEXT */
	u_printf(ERR,"mem_malloc: could not allocate %d bytes, mem_used:%d\n", size, ram_used);
	//MEM_STATS_INC(err);
	INC_MONITOR_ITEM_VALUE(mem_err);
	LWIP_MEM_ALLOC_UNPROTECT();
	mem_slide_check(FALSE);
...
void *mem_malloc(mem_size_t size)
{
	...
	while (local_mem_free_count != 0)
		;
	#endif /* LWIP_ALLOW_MEM_FREE_FROM_OTHER_CONTEXT */
	u_printf(DBG,"mem_malloc: could not allocate %" S16_F " bytes\n", (s16_t)size);
	//MEM_STATS_INC(err);
	INC_MONITOR_ITEM_VALUE(mem_err);
	LWIP_MEM_ALLOC_UNPROTECT();
	return NULL;
...
//				u1_send.sending.val = 0;
			...
			assert(alloc_mem_range_ok(mem));
			u_printf(ERR,"err mem, addr:%x, next:%x, prev:%x", mem, &ram[mem->next], &ram[mem->prev]);
			assert((mem->next) <= MEM_SIZE_ALIGNED);
			assert((mem->prev) < MEM_SIZE_ALIGNED);
			assert((mem->used == 0) || (mem->used == 1));
			assert(const_data_range_ok(mem->name));
...
//			u1_send.sending.val = 0;
		...
		if (mem->used)
		{
			...
			{
//				u1_send.sending.val = 0;
				u_printf(ERR,"detect memory overflow, mem_size:%d, flag:%x", (uint32_t)((uint32_t*) &ram[(uint32_t)p_char]) - (uint32_t)mem - SIZEOF_STRUCT_MEM, *(uint32_t*) &ram[(uint32_t)p_char]);
				u_printf(ERR,"mem_head:%x, data_addr:%x, last addr:%x, next_addr:%x", mem, (uint32_t)mem + SIZEOF_STRUCT_MEM, &ram[mem->prev], &ram[mem->next]);
				if (USE_MEM_DEBUG)
				{
					char name[32];
					struct mem *tmp_mem;

					u_printf(ERR,"critical addr:%x", ((uint32_t*) &ram[(uint32_t)p_char]));

					snprintf(name, 32, "%s", mem->name);
					u_printf(ERR,"name:%s", name);

					tmp_mem = (struct mem*) &ram[mem->prev];
					snprintf(name, 32, "%s", tmp_mem->name);
					u_printf(ERR,"last name:%s", name);

					tmp_mem = (struct mem*) &ram[mem->next];
					snprintf(name, 32, "%s", tmp_mem->name);
					u_printf(ERR,"next name:%s", name);
				}
				while (1)
					;
			}
