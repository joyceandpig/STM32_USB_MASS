...
void destroy_workqueue(struct workqueue_struct *workqueue)
{
	if(!workqueue)
		return;
	u_printf(DBG,"destroy %s", workqueue->name);
...
static void os_workqueue_thread(struct workqueue_struct *workqueue)
{
	int ret;
	uint32_t irq_flag;
	u_printf(DBG,"start workqueue %s id:%d", workqueue->name, workqueue->task_id);
	while (1)
	{
		...
		if (ret != 0)
		{
			...
			{
				u_printf(ERR,"os_tasklet_thread wait err:%d, %x\n", ret, workqueue->thread_event);
				sleep(2000);
			}
			continue;
		}
