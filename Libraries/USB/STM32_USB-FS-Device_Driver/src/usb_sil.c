...
//  return DataLength;
	 ...
	 if (_GetENDPOINT(bEpAddr) & EP_DTOG_TX)
  {
    /*read from ENDP2_BUF0Addr buffer*/
    DataLength = _GetEPDblBuf0Count(bEpAddr);
    PMAToUserBufferCopy(pBufferPointer, ENDP2_RXADDR, DataLength);
  }
  else
  {
    /*read from ENDP2_BUF1Addr buffer*/
    DataLength = _GetEPDblBuf1Count(bEpAddr);
    PMAToUserBufferCopy(pBufferPointer, ENDP2_TXADDR, DataLength);
  }
	ToggleDTOG_TX(bEpAddr);
	return DataLength;
}
