...
#ifdef SDIO_SD_CARD
	case SDIO_SD_CARD:
				result = SD_Init();  //SD¿¨½Ó¿Ú³õÊ¼»¯
				result = SD_GetCardInfo(&SDCardInfo); //»ñÈ¡SD¿¨ÐÅÏ¢
				result = SD_SelectDeselect((uint32_t) (SDCardInfo.RCA << 16));
				result = SD_EnableWideBusOperation(SDIO_BusWide_1b);//ÉèÖÃSDIO½Ó¿ÚÊý¾Ý¿í¶È
				result = SD_SetDeviceMode(SD_DMA_MODE);//ÉèÖÃ¹¤×÷Ä£Ê½
		return 0;
#endif
