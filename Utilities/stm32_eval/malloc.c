...
//*src:源地址
//n:需要复制的内存长度(字节为单位)
void mal_mymemcpy(void *des,void *src,u32 n)  
{  
    u8 *xdes=des;
		u8 *xsrc=src; 
    while(n--)*xdes++=*xsrc++;  
...
void mal_mymemset(void *s,u8 c,u32 count)  
{  
    u8 *xs = s;  
    while(count--)*xs++=c;  
}	   
...
{  
    mal_mymemset(mallco_dev.memmap, 0,memtblsize*2);//内存状态表数据清零  
	mal_mymemset(mallco_dev.membase, 0,memsize);	//内存池所有数据清零  
...
//释放内存(外部调用) 
//ptr:内存首地址 
void mal_myfree(void *ptr)  
{  
	u32 offset;  
    if(ptr==NULL)return;//地址为0.  
 	offset=(u32)ptr-(u32)mallco_dev.membase;  
    mal_mem_free(offset);	//释放内存     
}  
//分配内存(外部调用)
//size:内存大小(字节)
//返回值:分配到的内存首地址.
void *mal_mymalloc(u32 size)  
{  
    u32 offset;  									      
	offset=mal_mem_malloc(size);  	   				   
...
//size:要分配的内存大小(字节)
//返回值:新分配到的内存首地址.
void *mal_myrealloc(void *ptr,u32 size)  
{  
    u32 offset;  
    offset=mal_mem_malloc(size);  
    if(offset==0XFFFFFFFF)return NULL;     
    ...
    {  									   
	    mal_mymemcpy((void*)((u32)mallco_dev.membase+offset),ptr,size);	//拷贝旧内存内容到新内存   
        mal_myfree(ptr);  											  	//释放旧内存
