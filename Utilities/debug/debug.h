...
#define MyPrintf(...)		printf(__VA_ARGS__)/*指定输出,默认使用串口作为输出。若使用其他通信接口,需先实现MyPrintf函数*/

extern unsigned int os_time_get(void); /*获取系统运行时间,单位ms.需用户实现*/

#ifdef	OS_UCOS
	#define GET_SYSTEM_TIME			os_time_get() /*os_time_get()*/ /*获取系统运行时间函数*/
#else
	#define GET_SYSTEM_TIME			0					/*系统时间*/
	#warning	"GET_SYSTEM_TIME is not defined"
#endif
...
#ifdef	RELEASE_VERSION		//正式版本则取消调试
	#undef DEBUG					/*释放掉DEBUG*/
#else
	#define DEBUG
...
#define CLC_CMD	do{	\
		...
		{	\
			u8 i =0;	\
			for( ;i< sizeof("[sunsheen@Terminal]") ;i++)	\
			{\
